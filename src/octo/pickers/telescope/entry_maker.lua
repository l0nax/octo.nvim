...
function M.gen_from_issue(max_number, print_repo)
  ...
  return function(obj)
    ...
    local filename
    if kind == "issue" then
      filename = utils.get_issue_uri(obj.repository.nameWithOwner, obj.iid)
    else
      filename = utils.get_pull_request_uri(obj.repository.nameWithOwner, obj.iid)
    end
    return {
      filename = filename,
      kind = kind,
      value = obj.iid,
      ordinal = obj.iid .. " " .. obj.title,
